...
export default function ImportQuestionsPage() {
  ...
  const needsOptions = (type: QuestionType) => ['dropdown', 'radio', 'multiselect'].includes(type);
  ...
  const updateOption = (index: number, value: string) => {
    setFormData((prev) => {
      ...
      // Update optionValues key if the option name changed
      const newOptionValues = { ...prev.optionValues };
      if (oldOption !== value && newOptionValues[oldOption]) {
        newOptionValues[value] = newOptionValues[oldOption];
        delete newOptionValues[oldOption];
      }
      return { ...prev, options: newOptions, optionValues: newOptionValues };
    });
  };

  const updateOptionValue = (option: string, value: string) => {
    setFormData((prev) => ({
      ...prev,
      optionValues: { ...prev.optionValues, [option]: value },
    }));
  };
